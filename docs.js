...
/* NOVO: PDF da Ficha Clínica */
export async function docClinicalRecord({ patient, encounter }){
  const pro = await getPro();
  const today = new Date().toLocaleDateString("pt-BR");

  const dob = patient?.dob ? new Date(patient.dob+"T00:00:00").toLocaleDateString("pt-BR") : "";
  const headerPatient = [
    patient?.name ? `Paciente: ${patient.name}` : "Paciente:",
    patient?.phone ? `Contato: ${patient.phone}` : "",
    dob ? `Nascimento: ${dob}` : ""
  ].filter(Boolean).join(" • ");

  const apptISO = encounter?.date || new Date().toISOString().slice(0,10);
  const apptBR = new Date(apptISO + "T00:00:00").toLocaleDateString("pt-BR");

  const body =
`Data do atendimento: ${apptBR}
...
`;
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">${baseDocCSS()}</head><body>
    <div class="page"><div class="frame">
      ...
      <div class="patient"><strong>${esc(headerPatient)}</strong></div>
      <div class="title">Ficha clínica</div>
      <div class="textblock">${esc(body)}</div>

      <div class="footer">
        <div class="row">
          <div class="sig">Assinatura</div>
          <div class="sig">Data: ${esc(today)}</div>
        </div>
        <div class="addr">${esc(pro.addr||"")}</div>
      </div>
    </div></div>
