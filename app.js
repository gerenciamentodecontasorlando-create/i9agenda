...
import { docPrescription, docSimple, docScheduleDay, docClinicalRecord } from "./docs.js";
...
async function loadPatientHistory(patientId){
  if(!patientId){
    $("patientSummary").textContent = "Selecione um paciente na agenda.";
    $("historyList").innerHTML = "";
    return;
  }

  const p = await DB.get("patients", patientId);
  const dob = p?.dob ? new Date(p.dob+"T00:00:00").toLocaleDateString("pt-BR") : "";
  const summary = [p?.name || "", p?.phone || "", dob].filter(Boolean).join(" • ");
  $("patientSummary").textContent = summary || "Paciente selecionado.";

  const encs = await DB.getAllByIndex("encounters", "patientId", patientId);
  encs.sort((a,b) => (b.updatedAt||"").localeCompare(a.updatedAt||""));
  ...
  if(encs.length === 0){
    ...
    li.innerHTML = `<div class="left"><strong>Nenhum histórico ainda</strong><span>Salve um atendimento para começar.</span></div>`;
    ul.appendChild(li);
    return;
  }
  ...
  for(const e of encs){
    ...
    const d = e.date ? new Date(e.date+"T00:00:00").toLocaleDateString("pt-BR") : "";
    const title = document.createElement("strong");
    title.textContent = `Atendimento — ${d}`;

    const preview = document.createElement("span");
    preview.textContent = (e.notes || "").replace(/\s+/g," ").trim().slice(0, 90) || "(sem notas)";
    ...
    btnLoad.onclick = async () => {
      ...
      $("rxText").value = e.rxText || "";
      setAutosave("carregado");
      toast("Atendimento carregado no editor.");
    };
...
async function selectAppointment(apptId){
  ...
  await loadPatientHistory(state.selectedPatientId);
...
function shiftDay(delta){
  ...
  if(state.calendar) state.calendar.rerender();
  refreshAgenda();
}
...
async function init(){
  ...
  // PDF Ficha clínica
  $("btnDocClinical").onclick = async () => {
    if(!state.selectedApptId){
      toast("Abra um item da agenda (Abrir) para imprimir a ficha clínica.");
      return;
    }
    const appt = await DB.get("appointments", state.selectedApptId);
    if(!appt) return;
    ...
    await docClinicalRecord({ patient, encounter });
  };
  ...
  $("btnRefreshHistory").onclick = () => loadPatientHistory(state.selectedPatientId);

  // Calendário mensal (clicável)
  state.calendar = mountCalendar({
    getSelectedISO: () => state.dateISO,
    setSelectedISO: (iso) => { state.dateISO = iso; $("datePicker").value = iso; },
    onChange: () => refreshAgenda()
  });
